...
유저 프롬프트 (USER_PROMPT 변수):
  ...
  - scriptのサインオフはコードが自動追加するため、scriptに含めないこと
    ※ 사인오프 정본 단일 출처: step3_chatgpt.py 상수
      SIGNOFF_DEFAULT_KO = "이상, 모찌엔이었습니다!"          ← KO 대본용
      SIGNOFF_DEFAULT_JA = "以上、モチエンがお伝えしました！" ← 펀넬 미매칭 시
      SIGNOFF_FUNNEL_JA  = "もっと深く知るには、コメント欄へ。以上、モチエンでした。" ← 펀넬 매칭 시
