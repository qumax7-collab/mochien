...
================================================================
✅  세션 6 — 쇼츠 UI 품질 3종 (2026-06-21)
================================================================

  [작업 1] 자막 의미단위 분절 — step7_whisper_subtitle.py (커밋 461b62e)
        MAX_LINE_CHARS=8 고정 하드컷 → 일본어 조사·구두점 기반 의미단위 분절로 전면 교체.
        MAX_LINE_CHARS는 wrap_text() 시각적 줄바꿈 전용으로 역할 축소.

        [핵심 로직]
          · PARTICLES = frozenset("はがをにへとのもでねよわ") — 12종 (や·か 동사어간 중복으로 제외)
          · MIN_PARTICLE_FLUSH_LEN = 3 — 3글자 미만 조각은 조사 flush 억제 (의미없는 분절 방지)
          · 3중 억제 조건: がる/がり 어미(上がる 등) / のか 문말 구문 / 최소 길이 미달
          · _absorb_lone_punct(): 0ms 단독 구두점(。、！？) 세그먼트를 앞 세그먼트에 흡수
          · 숫자 토큰 보호: 1.4%·値上がり·値下がり 등을 조사로 오인식하지 않도록 분기 처리
          · wrap_text() 내 후방 스캔: 조사+숫자어미 복합 감지 + がり/がる 보호

        [백로그]
          · MeCab 형태소 분석기 도입 — 気になる 류 복합동사 어근 분리 근본 해결
          · 폰트 135px 확대 + MAX_LINE_CHARS 재조정 (분절 변경 후 폭 재검토 필요)

  [작업 2] 쇼츠 상단 바 다이나믹 아일랜드 대응 — step6_ffmpeg.py (커밋 bc71020)
        iPhone Dynamic Island(아이폰 15 Pro 등 상단 노치) 안전선 확보.
        네이비 바를 y=300 아래로 내려 0~299px를 배경 영상으로 노출.
        타이틀 텍스트 중심 Y=396px 고정(폰으로 통과 확인).

        [상수 — 최종]
          TOP_BAR_Y    = 300   # 네이비 바 시작 Y (0~299px 배경 노출 / 아일랜드 안전선)
          TOP_BAR_H    = 192   # 바 높이 → 범위 y=300~492
          RED_LINE_H   = 5
          TITLE_CENTER_Y = 396 # 타이틀 중심 Y 고정 (바 수직중앙 수식 아님)

        [레이아웃]
          y=0~299   → 배경 영상 노출 (다이나믹 아일랜드 관통 허용)
          y=300~492 → 네이비 바
          y=492~497 → 레드라인
          y=396     → 타이틀 텍스트 중심 (바 내부이지만 아일랜드 하단)

        [진행 이력]
          A안(바 y=0, 높이 492) 시도 → 폰 확인 후 바 내려 위 비움으로 최종 복귀
          TITLE_CENTER_Y=396는 A안·복귀안 모두 유지 (폰 통과 좌표)

  [작업 3] 썸네일 thumb_headline 수치 검증 게이트 — step8_thumbnail.py (커밋 68d9394)
        GPT가 소수점 누락 등으로 왜곡된 수치를 thumb_headline에 삽입하는 버그 차단.
        예: CPI 1.4% → GPT 출력 「5月物価14安定」 → "14"가 script에 없음 → sys.exit(1).

        [구현]
          THUMB_NUM_RE  = re.compile(r'\d+(?:\.\d+)?%?')
          FULLWIDTH_TABLE — 전각숫자·% 정규화 (１→1, ％→%)
          extract_num_tokens(text) → set[str]
          validate_thumb_numerics(gpt):
            · thumb_headline 숫자 없으면(워드형) 무조건 통과
            · 있으면 script+hook 수치 집합과 토큰 단위 비교
            · 불일치 → sys.exit(1) + 텔레그램 알림 (영상명·thumb값·불일치 수치 명시)
            · 삽입 위치: generate_ai_bg() 직전 (OpenAI Images API 비용 발생 전 차단)

        [테스트 (_verify_thumb_validate.py)]
          Case A: 원본 「5月物価14安定」→ {"5","14"} / ref에 "14" 없음 → sys.exit(1) ✅
          Case B: thumb를 「1.4%安定」→ {"1.4%"} / ref에 "1.4%" 있음 → 통과 ✅
          텔레그램 실수신 확인 완료

✅  기사 소스 다양화 — RSS 섹션 확장 + 경제 키워드 소프트 정렬 (2026-06-23)
        [배경] 웹 UI 기사 선택 시 후보 주제가 시장·거시(엔/금리/주가)로만 수렴.
          원인 ① cat2 오라벨(코드 주석 "生活·社会政策" ↔ 실제 文化・エンタメ)
               ② 경제 키워드 게이트 협소(10개 전부 시장·거시 계열)
        [step2_select.py — 4개 변경 / fetch_articles() 공용 함수]
          · RSS_URLS: Yahoo 2개(business/domestic) 제거 → 전 소스 NHK 공영방송 통일
            cat2(文化・エンタメ, 오라벨) 제거 → cat4 政治 + cat1 社会 추가 (주석 실제 채널명 정정)
          · ECONOMIC_KEYWORDS: 10 → 26개 (생활·정책 축 보강: 雇用/賃金/賃上げ/年金/税金/補助金/
            景気/倒産/値上げ/消費/貿易/輸出/輸入/エネルギー/電気代/ガソリン)
            ※ 税(→税金으로 좁힘)·価格·企業는 노이즈 우려로 제외
          · 게이트 → 소프트 정렬 전환: 경제 기사 상위 + 비경제 하위(둘 다 후보 노출)
            경제 기사는 항상 상위라 묻히지 않으면서 정책·생활 주제도 선택지로 노출
          · BLOCKED_KEYWORDS: NISA·iDeCo 차단 해제(정책·세제 뉴스 소재 확보) / 투자 권유성만 잔존
            (Yahoo 제거로 클릭베이트 소스 소멸 / NHK엔 종목추천·코인투기 콘텐츠 없음)
            ※ 暗호화폐류는 40~60대 채널 성격상 차단 유지
        [적용 범위] fetch_articles() 수정 → 웹 UI + 자동(텔레그램 단독/일괄) 전부 자동 반영
