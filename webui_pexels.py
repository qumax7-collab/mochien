...
def load_used_video_urls() -> set:
    """최근 RETENTION_DAYS일 이내 사용된 영상 URL 세트 반환."""
    cutoff = datetime.now(JST).timestamp() - RETENTION_DAYS * 86400
    return {
        entry["url"]
...
def save_used_video(url: str, thumb: str = ""):
    """사용된 영상 URL을 used_videos.json에 누적 저장."""
    entries = [
        e for e in _load_raw()
        if e.get("url") != url  # 중복 제거 후 최신으로 재삽입
    ]
    entries.append({
        "url":   url,
        "thumb": thumb,
        "ts":    datetime.now(JST).timestamp(),
    })
    # 30일 초과 항목 정리
    cutoff  = datetime.now(JST).timestamp() - RETENTION_DAYS * 86400
...
# ── Pexels API 호출 ───────────────────────────────────────
def fetch_pexels_candidates(query: str, count: int = 6) -> list:
    """
    ...
    while len(candidates) < count and page <= 3:
    ...
    # 후보가 부족하고 fallback query 시도
    if len(candidates) < count and query != "japanese economy":
        extras = fetch_pexels_candidates("japanese economy", count - len(candidates))
        # 중복 URL 방지
