...
def main():
    ...
    # step9 완료 후 검수 — 실패해도 파이프라인 계속 진행
    print(f"\n{'='*50}")
    print("[step10] step10_gemini_review.py --mode shorts 실행 중...")
    print(f"{'='*50}")
    r10 = subprocess.run([sys.executable, "step10_gemini_review.py", "--mode", "shorts"])
    if r10.returncode != 0:
